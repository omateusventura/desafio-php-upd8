  ...
  <main class="w-screen h-screen bg-slate-50 py-4 px-4">
    ...
    <div class="max-w-7xl mx-auto bg-white mt-6 border border-slate-100 shadow-md rounded-md px-4 py-6">
      <form id="customerForm" class="grid grid-cols-12 gap-3">
        <input id="inputID" type="hidden" value="{{ $customer->id }}" />

        <div class="relative col-span-12 md:col-span-3">
          <input
            type="text"
            id="inputCPF"
            class="input-check block px-2.5 pb-2.5 pt-4 w-full border text-sm text-gray-900 bg-transparent rounded-lg appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer disabled:bg-slate-100"
            name="cpf"
            placeholder=" "
            autocomplete="off"
            value="{{ $customer->cpf }}"
            disabled
          />
          <label for="inputCPF" class="input-check absolute text-sm text-gray-500 duration-300 transform -translate-y-4 scale-75 top-4 z-10 origin-[0] px-2 peer-focus:px-2 peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:top-1/2 peer-focus:top-4 peer-focus:scale-75 peer-focus:-translate-y-4 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto start-1">
            CPF
          </label>
        </div>
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="flex items-center gap-2">
              <input
                ...
                name="genere"
                value="female"
                {{ $customer->genere === 'female' ? 'checked' : '' }}
              />
        ...
        <div class="relative col-span-12 md:col-span-3">
          ...
          >
            ...
            <option value="AC" {{ $customer->state === 'AC' ? 'selected' : '' }}>Acre</option>
            <option value="AL" {{ $customer->state === 'AL' ? 'selected' : '' }}>Alagoas</option>
            <option value="AP" {{ $customer->state === 'AP' ? 'selected' : '' }}>Amapá</option>
            <option value="AM" {{ $customer->state === 'AM' ? 'selected' : '' }}>Amazonas</option>
            <option value="BA" {{ $customer->state === 'BA' ? 'selected' : '' }}>Bahia</option>
            <option value="CE" {{ $customer->state === 'CE' ? 'selected' : '' }}>Ceará</option>
            <option value="DF" {{ $customer->state === 'DF' ? 'selected' : '' }}>Distrito Federal</option>
            <option value="ES" {{ $customer->state === 'ES' ? 'selected' : '' }}>Espírito Santo</option>
            <option value="GO" {{ $customer->state === 'GO' ? 'selected' : '' }}>Goiás</option>
            <option value="MA" {{ $customer->state === 'MA' ? 'selected' : '' }}>Maranhão</option>
            <option value="MT" {{ $customer->state === 'MT' ? 'selected' : '' }}>Mato Grosso</option>
            <option value="MS" {{ $customer->state === 'MS' ? 'selected' : '' }}>Mato Grosso do Sul</option>
            <option value="MG" {{ $customer->state === 'MG' ? 'selected' : '' }}>Minas Gerais</option>
            <option value="PA" {{ $customer->state === 'PA' ? 'selected' : '' }}>Pará</option>
            <option value="PB" {{ $customer->state === 'PB' ? 'selected' : '' }}>Paraíba</option>
            <option value="PR" {{ $customer->state === 'PR' ? 'selected' : '' }}>Paraná</option>
            <option value="PE" {{ $customer->state === 'PE' ? 'selected' : '' }}>Pernambuco</option>
            <option value="PI" {{ $customer->state === 'PI' ? 'selected' : '' }}>Piauí</option>
            <option value="RJ" {{ $customer->state === 'RJ' ? 'selected' : '' }}>Rio de Janeiro</option>
            <option value="RN" {{ $customer->state === 'RN' ? 'selected' : '' }}>Rio Grande do Norte</option>
            <option value="RS" {{ $customer->state === 'RS' ? 'selected' : '' }}>Rio Grande do Sul</option>
            <option value="RO" {{ $customer->state === 'RO' ? 'selected' : '' }}>Rondônia</option>
            <option value="RR" {{ $customer->state === 'RR' ? 'selected' : '' }}>Roraima</option>
            <option value="SC" {{ $customer->state === 'SC' ? 'selected' : '' }}>Santa Catarina</option>
            <option value="SP" {{ $customer->state === 'SP' ? 'selected' : '' }}>São Paulo</option>
            <option value="SE" {{ $customer->state === 'SE' ? 'selected' : '' }}>Sergipe</option>
            <option value="TO" {{ $customer->state === 'TO' ? 'selected' : '' }}>Tocantins</option>
          </select>
